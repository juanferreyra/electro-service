...
public class TipoProductoDAO {
	...
	private static final String findByDetalle = "SELECT id, detalle, idusuario FROM tipo_producto WHERE habilitado = true AND detalle = ?;";

	private static Conexion conexion = Conexion.getConexion();

	public TipoProductoDTO findByDetalle(String detalle) {
		...
		try {
			...
			while (resultSet.next()) {
				tipo = new TipoProductoDTO(resultSet.getInt("id"), resultSet.getString("detalle"),
	...
	public List<TipoProductoDTO> readAll() {
		...
		try {
			...
			while (resultSet.next()) {
				tipos_productos.add(new TipoProductoDTO(resultSet.getInt("id"), resultSet.getString("detalle"),
						resultSet.getInt("idusuario")));
			}
		} catch (SQLException e) {
			e.printStackTrace();
	...
	public TipoProductoDTO find(int idtipo_producto) {
		...
		try {
			...
			while (resultSet.next()) {
				tipo_producto = new TipoProductoDTO(resultSet.getInt("id"), resultSet.getString("detalle"),
						resultSet.getInt("idusuario"));
			}
		} catch (SQLException e) {
			e.printStackTrace();
